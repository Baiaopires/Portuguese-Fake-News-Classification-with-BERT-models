...
############################################################################################################################################################################################################################################################
  ...
  if((sum(val_accuracy)/len(val_accuracy)) > best_val_accuracy):
    ...
    bool_value = 1
    model.save_pretrained("./Models/Fakepedia_on_BERTModel_FineTunned_Model_epoch_{0}_val_acc_{1}".format(best_epoch, best_val_accuracy))
...
plt.savefig("./Plots/Plot_Fakepedia_Dataset_On_BERTModel_val_acc_{0}.png".format(best_val_accuracy), bbox_inches='tight')
plt.show()
...
model = ppb.AutoModelForSequenceClassification.from_pretrained(
    "./Models/Fakepedia_on_BERTModel_FineTunned_Model_epoch_{0}_val_acc_{1}".format(best_epoch, best_val_accuracy),
    num_labels = 2,
    output_attentions = False,
    output_hidden_states = False
)
