...
df = pd.read_csv('https://raw.githubusercontent.com/Baiaopires/datasets_used/main/datasets_used/Fakepedia/Fakepedia.csv', delimiter=';', skiprows = lambda x: x in [0], header=None)
df.drop(columns=[0, 1, 3, 4, 5, 6, 7, 8, 9, 10], inplace=True)
df = df.rename(columns={2: 0, 11: 1})
index = df[0].index[df[0].apply(pd.isnull)]
...
df = pd.concat([df[0].append(df[1].iloc[:]).reset_index(drop=True)])
df = df.to_frame()
df[1] = df.loc[:, 0]
...
############################################################################################################################################################################################################################################################
  ...
  if((sum(val_accuracy)/len(val_accuracy)) > best_val_accuracy):
    ...
    bool_value = 1
    model.save_pretrained("./Models/Fakepedia_on_BERTimbau_FineTunned_Model_epoch_{0}_val_acc_{1}".format(best_epoch, best_val_accuracy))
...
plt.savefig("./Plots/Plot_Fakepedia_Dataset_On_BERTimbau_val_acc_{0}.png".format(best_val_accuracy), bbox_inches='tight')
plt.show()

############################################################################################################################################################################################################################################################

model = ppb.AutoModelForSequenceClassification.from_pretrained(
    "./Models/Fakepedia_on_BERTimbau_FineTunned_Model_epoch_{0}_val_acc_{1}".format(best_epoch, best_val_accuracy),
    num_labels = 2,
    output_attentions = False,
    output_hidden_states = False
)
